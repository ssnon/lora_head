...
class CustomBertSelfAttention(models.bert.modeling_bert.BertSelfAttention):
    def __init__(self, config, original_layer):
        ...
        for i in range(self.num_attention_heads):
            layerA = nn.Linear(config.hidden_size, r_).to(device)
            nn.init.normal_(layerA.weight, mean=0, std=0.01)
            nn.init.zeros_(layerA.bias)
            layerB = nn.Linear(r_, self.attention_head_size).to(device)
            nn.init.zeros_(layerB.weight)
            nn.init.zeros_(layerB.bias)
            A_q.append(layerA)
            B_q.append(layerB)
        ...
        self.A_v = nn.Sequential(*A_v)
        self.B_v = nn.Sequential(*B_v)

        '''
        self.A_q = nn.Linear(r_, config.hidden_size)
        nn.init.normal_(self.A_q.weight, mean=0, std=0.01)
        nn.init.zeros_(self.A_q.bias)
        self.B_q = nn.Linear(self.attention_head_size, r_)
        nn.init.zeros_(self.B_q.weight)
        nn.init.zeros_(self.B_q.bias)
        self.A_v = nn.Linear(r_, config.hidden_size)
        nn.init.normal_(self.A_v.weight, mean=0, std=0.01)
        nn.init.zeros_(self.A_v.bias)
        self.B_v = nn.Linear(self.attention_head_size, r_)
        nn.init.zeros_(self.B_v.weight)
        nn.init.zeros_(self.B_v.bias)
        '''
    ...
    def forward(
        ...
        new_value_layer = []
        for i in range(self.num_attention_heads):
            new_query_layer_A = self.A_q[i](hidden_states)
            new_query = self.B_q[i](new_query_layer_A)
            new_query_layer.append(new_query)
        for i in range(self.num_attention_heads):
            new_value_layer_A = self.A_v[i](hidden_states)
            new_value = self.B_v[i](new_value_layer_A)
            new_value_layer.append(new_value)
            
        new_combined_query = torch.cat(new_query_layer, dim=2)
        new_combined_query = new_combined_query * self.scaling
        new_combined_value = torch.cat(new_value_layer, dim=2)
        new_combined_value = new_combined_value * self.scaling
...
class MyCallback:
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        num = 0
        lora_A_default_weights = model.bert.encoder.layer[7].attention.self.A_v[5].weight
        lora_B_default_weights = model.bert.encoder.layer[7].attention.self.B_v[5].weight
        ...
        weight_path = f'/mnt/hdd/weight/head_40000/weight_{epoch}.json'
